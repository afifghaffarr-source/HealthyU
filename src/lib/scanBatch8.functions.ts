...
export const notifyUser = createServerFn({ method: "POST" })
  ...
  .inputValidator((d: { targetUserId: string; type: string; title: string; body?: string; link?: string }) =>
    z.object({
...
export const markNotifRead = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    const { supabase, userId } = context;
    await supabase.from("notifications_log").update({ read: true }).eq("id", data.id).eq("user_id", userId);
    return { ok: true };
  });
...
export const transcribeMoodVoice = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: { audioBase64: string }) => z.object({ audioBase64: z.string().min(10) }).parse(d))
  .handler(async ({ data }) => {
...
export const adjustPortion = createServerFn({ method: "POST" })
  ...
  .inputValidator((d: { baseCalories: number; multiplier: number }) =>
    z.object({ baseCalories: z.number().positive(), multiplier: z.number().min(0.1).max(5) }).parse(d)
  )
  .handler(async ({ data }) => {
...
export const restaurantsNearby = createServerFn({ method: "POST" })
  ...
  .inputValidator((d: { lat: number; lng: number; radiusKm?: number }) =>
    z.object({ lat: z.number(), lng: z.number(), radiusKm: z.number().min(0.5).max(20).optional() }).parse(d)
  )
  .handler(async ({ context, data }) => {
...
export const redeemFamilyInvite = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: { token: string }) => z.object({ token: z.string().min(5).max(100) }).parse(d))
  .handler(async ({ context, data }) => {
    const { supabase, userId } = context;
    const { data: inv } = await supabase.from("family_invites").select("*").eq("token", data.token).maybeSingle();
    ...
    await supabase.from("family_invites").update({ used_by: userId, used_at: new Date().toISOString() }).eq("id", inv.id);
...
export const createDoctorReferral = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: { reason: string }) => z.object({ reason: z.string().min(5).max(500) }).parse(d))
  .handler(async ({ context, data }) => {
    const { supabase, userId } = context;
...
export const logMeditation = createServerFn({ method: "POST" })
  ...
  .inputValidator((d: { durationMin: number; type?: string }) =>
    z.object({ durationMin: z.number().int().min(1).max(120), type: z.string().max(30).optional() }).parse(d)
  )
  .handler(async ({ context, data }) => {
    const { supabase, userId } = context;
