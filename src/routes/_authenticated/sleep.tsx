...
function SleepPage() {
  ...
  const chartData = [...last7]
    ...
    .map((l) => ({
      ...
      hours: Number(
        ((new Date(l.sleep_end).getTime() - new Date(l.sleep_start).getTime()) / 3600000).toFixed(
          1,
        ),
      ),
      score: l.quality * 20,
      ...
      <div className="max-w-md mx-auto px-5 pt-2 space-y-5">
        ...
        <section className="space-y-2 animate-fade-up">
          ...
          {logs.length === 0 ? (
            ...
            logs.map((l) => {
              const hrs =
                (new Date(l.sleep_end).getTime() - new Date(l.sleep_start).getTime()) / 3600000;
              return (
                <div
                  key={l.id}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-sm">
                      {hrs.toFixed(1)} jam · {"★".repeat(l.quality)}
                    </p>
                    <p className="text-xs text-muted-foreground">
                      {new Date(l.sleep_start).toLocaleString("id-ID", {
                        day: "numeric",
                        month: "short",
                        hour: "2-digit",
                        minute: "2-digit",
