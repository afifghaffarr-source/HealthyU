...
function ArticleReader() {
  ...
  const share = async () => {
    const data = { title: `Artikel #${id}`, text: "Baca artikel sehat ini di HealthyU", url: window.location.href };
  ...
  return (
    <div className="min-h-screen pb-28 px-4">
      <div className="fixed top-0 inset-x-0 z-40 h-1 bg-transparent">
        <div className="h-full bg-gradient-to-r from-primary to-accent transition-[width]" style={{ width: `${progress}%` }} />
      </div>
      <TopAppBar
        title="Artikel"
        ...
        action={
          <div className="flex items-center gap-1">
            <button onClick={share} aria-label="Bagikan" className="inline-flex size-9 items-center justify-center rounded-full hover:bg-muted">
              <Share2 className="size-5" />
            </button>
            <button onClick={toggleSave} aria-label={saved ? "Hapus bookmark" : "Simpan bookmark"} className="inline-flex size-9 items-center justify-center rounded-full hover:bg-muted">
