...
function Page() {
  ...
  const avgDur = avg7.length
    ? avg7.reduce(
        (a, e) => a + (e.bedtime && e.wake_time ? durationHours(e.bedtime, e.wake_time) : 0),
        0,
      ) / avg7.length
    : 0;
        ...
        <section className="rounded-3xl p-5 bg-gradient-to-br from-indigo-500 via-violet-500 to-purple-600 text-white outline-1 outline-black/5">
          ...
          <div className="mt-3 flex items-center justify-between text-xs">
            <span className="inline-flex items-center gap-1">
              <Moon className="size-3" /> {bt}
            </span>
            <span className="inline-flex items-center gap-1">
              <Sunrise className="size-3" /> {wt}
            </span>
          </div>
        </section>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2.5 rounded-xl outline-1 outline-black/10 bg-background"
              />
            </div>
            <div>
            ...
            <div className="flex items-center justify-between text-[11px] font-semibold">
              ...
              <span className="inline-flex items-center gap-0.5">
                {Array.from({ length: 5 }).map((_, i) => (
                  <Star
                    key={i}
                    className={`size-3.5 ${i < q ? "fill-amber-400 text-amber-400" : "text-muted-foreground/30"}`}
                  />
                ))}
              </span>
            </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="inline-flex items-center gap-0.5">
                    {Array.from({ length: 5 }).map((_, i) => (
                      <Star
                        key={i}
                        className={`size-3 ${i < (e.quality ?? 0) ? "fill-amber-400 text-amber-400" : "text-muted-foreground/30"}`}
                      />
                    ))}
                  </span>
                </div>
