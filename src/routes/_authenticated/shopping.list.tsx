...
import { smartShoppingList } from "@/features/scan/lib/scanBatch8.functions";
    ...
    <div className="min-h-dvh pb-24 bg-background">
      ...
      <main className="max-w-md mx-auto px-4 pt-4 space-y-4">
        <textarea
          ...
          rows={5}
          placeholder="1 bahan per baris..."
          className="w-full px-3 py-2 rounded-xl border bg-card text-sm"
        />
        <button
          onClick={() => mut.mutate()}
          disabled={!input || mut.isPending}
          className="w-full rounded-xl bg-primary text-primary-foreground py-2.5"
        >
          Estimasi Harga
        </button>
        {mut.data && (
            ...
            {mut.data.items.map((it, i) => (
              <div key={i} className="flex justify-between text-sm">
                ...
                <span className="font-medium">
                  Rp {(it.price_idr ?? 0).toLocaleString("id-ID")}
                </span>
              </div>
            ))}
