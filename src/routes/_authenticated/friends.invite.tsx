...
function Page() {
  ...
  const create = useMutation({ mutationFn: () => createFn({ data: undefined as never }), onError: (e: Error) => toast.error(e.message) });
  const redeem = useMutation({
    mutationFn: () => redeemFn({ data: { token } }),
    onSuccess: () => toast.success("Berhasil menjadi teman!"),
    onError: (e: Error) => toast.error(e.message),
  });
  const link = create.data?.invite ? `${window.location.origin}/friends/invite?token=${(create.data.invite as any).token}` : "";
  return (
    <div className="min-h-dvh pb-24 bg-background">
      ...
      <main className="max-w-md mx-auto px-4 pt-4 space-y-4">
        <div className="rounded-2xl bg-card border p-4 space-y-2">
          ...
          <button onClick={() => create.mutate()} disabled={create.isPending} className="w-full rounded-lg bg-primary text-primary-foreground py-2 text-sm">Generate</button>
          {link && (
            <div className="space-y-2">
              <p className="text-xs break-all">{link}</p>
              <a href={`https://wa.me/?text=${encodeURIComponent("Yuk gabung HealthyU bareng aku! " + link)}`} target="_blank" rel="noreferrer" className="block text-center text-sm rounded-lg border py-2">Share via WhatsApp</a>
            </div>
          )}
        </div>
        <div className="rounded-2xl bg-card border p-4 space-y-2">
          ...
          <button onClick={() => redeem.mutate()} disabled={!token || redeem.isPending} className="w-full rounded-lg border py-2 text-sm">Tukar</button>
        </div>
      </main>
      <BottomNav />
    </div>
