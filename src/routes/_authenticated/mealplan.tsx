...
function MealPlanPage() {
  ...
  const [picker, setPicker] = useState<{ date: string; meal: (typeof MEAL_TYPES)[number]["id"] } | null>(null);
  ...
  const addMut = useMutation({
    mutationFn: (payload: { food_item_id: string | null; custom_name: string | null; calories: number }) =>
      add({
        data: {
    ...
    <main className="min-h-screen bg-background pb-28">
      <div className="max-w-md mx-auto px-5 pt-8 space-y-5">
        ...
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => setWeekStart(startOfWeek())} className="text-xs font-semibold text-muted-foreground">
            Minggu ini
          </button>
          <button
            onClick={() => {
        ...
        {days.map((d) => {
          ...
          return (
            <section key={dateStr} className="bg-card p-4 rounded-3xl outline-1 outline-black/5 animate-fade-up">
                  ...
                  return (
                    <div key={mt.id} className="flex items-start gap-2">
                      ...
                      <div className="flex-1">
                        <p className="text-[10px] font-bold uppercase text-muted-foreground tracking-wider">{mt.label}</p>
                        {items.length === 0 ? (
                          <button
                          ...
                          <div className="space-y-1">
                            {items.map((p) => (
                              <div key={p.id} className="flex items-center gap-2 text-sm">
                                ...
                                <span className="text-xs text-muted-foreground tabular-nums">{Math.round(Number(p.calories))} kcal</span>
                                <button onClick={() => delMut.mutate(p.id)} className="text-muted-foreground">
        ...
        >
          ...
          >
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={() => setPicker(null)}><X className="size-5" /></button>
            </div>
            <input
              autoFocus
              value={query}
            ...
            <div className="space-y-2">
              {foods.map((f) => (
                ...
                >
                  ...
                  <span className="text-xs text-muted-foreground tabular-nums">{Math.round(Number(f.calories))} kcal</span>
                </button>
              ))}
              {query && (
                <button
