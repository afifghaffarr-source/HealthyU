...
type Item = { id: string; item: string; qty: string; price: number; category: "Protein" | "Karbo" | "Sayur" | "Lainnya" };
const SAMPLE: Item[] = [
...
function ShoppingDerivePage() {
  ...
  const grouped = useMemo(() => {
    ...
    SAMPLE.forEach((i) => { (g[i.category] ??= []).push(i); });
    return g;
  }, []);
  ...
  return (
    <div className="min-h-screen pb-32 px-4 bg-background">
      ...
      <div className="mt-4 rounded-3xl bg-gradient-to-br from-primary to-accent text-primary-foreground p-4">
        <p className="text-xs uppercase tracking-wider opacity-80 inline-flex items-center gap-1"><Sparkles className="size-3" /> Diturunkan dari meal plan 7 hari</p>
        <p className="text-2xl font-bold mt-1 tabular-nums" style={{ fontFamily: "var(--font-display)" }}>
          Rp {total.toLocaleString("id-ID")}
        </p>
        <p className="text-xs opacity-90 mt-0.5">{doneCount}/{SAMPLE.length} item sudah dicentang</p>
      </div>

      {Object.entries(grouped).map(([cat, items]) => (
        <section key={cat} className="mt-5">
          <h3 className="text-xs uppercase tracking-wider font-bold text-muted-foreground mb-2 px-1">{cat}</h3>
                ...
                <li key={s.id}>
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <span className={`size-6 rounded-full grid place-items-center border-2 transition ${
                        done ? "bg-primary border-primary text-primary-foreground" : "border-muted-foreground/40"
                      }`}>
                        {done && <Check className="size-3.5" />}
                      </span>
                      <div>
                        <p className={`font-semibold text-sm ${done ? "line-through" : ""}`}>{s.item}</p>
                        <p className="text-xs text-muted-foreground">{s.qty}</p>
                      </div>
                    </div>
                    <span className="font-mono text-sm text-primary tabular-nums">Rp {s.price.toLocaleString("id-ID")}</span>
                  </button>
                </li>
              );
            })}
      ...
      <div className="mt-6 flex gap-2">
        <Link to="/shopping/list" className="flex-1 rounded-2xl bg-card border py-3 text-center text-sm font-semibold">
