...
function Page() {
  ...
  useEffect(() => {
    ...
    let raf = 0;
    const Detector = (window as unknown as { BarcodeDetector?: new (o: { formats: string[] }) => { detect: (s: HTMLVideoElement) => Promise<{ rawValue: string }[]> } }).BarcodeDetector;
    if (!Detector) { toast.error("BarcodeDetector tidak didukung browser ini"); return; }
      ...
      const tick = async () => {
        if (videoRef.current && videoRef.current.readyState >= 2) {
          try {
            ...
            if (res[0]) { setCode(res[0].rawValue); mut.mutate(res[0].rawValue); return; }
          } catch { /* noop */ }
        }
  ...
  return (
    <div className="min-h-dvh pb-24 bg-background">
      ...
      <main className="max-w-md mx-auto px-4 pt-4 space-y-3">
        ...
        {code && <p className="text-sm">Barcode: <span className="font-mono">{code}</span></p>}
        {mut.data?.product && <pre className="rounded-xl border bg-card p-3 text-xs overflow-auto">{JSON.stringify(mut.data.product, null, 2)}</pre>}
      </main>
      <BottomNav />
    </div>
  );
