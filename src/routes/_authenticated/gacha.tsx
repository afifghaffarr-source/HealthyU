    ...
    <div className="min-h-dvh pb-24 bg-background">
      ...
      <main className="max-w-md mx-auto px-4 pt-8 text-center space-y-6">
        <div className="relative size-44 mx-auto">
          <div className={`absolute inset-0 rounded-full bg-[conic-gradient(from_0deg,#16a34a,#0ea5e9,#f59e0b,#ec4899,#a855f7,#16a34a)] ${spinning ? "animate-spin" : ""}`} style={{ animationDuration: spinning ? "0.6s" : undefined }} />
          <div className="absolute inset-2 rounded-full bg-card grid place-items-center text-6xl">🎁</div>
        ...
        {last && !spinning && <p className="text-lg font-bold text-primary animate-fade-up">🎉 Hasil: {last}</p>}
      </main>
      <BottomNav />
    </div>
  );
