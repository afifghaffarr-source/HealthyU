    ...
    <div className="min-h-dvh pb-24 bg-background">
      ...
      <main className="max-w-md mx-auto px-4 pt-4 space-y-4 animate-fade-up">
        ...
        <div className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-wider text-muted-foreground">
          ...
          <span className="ml-auto tabular-nums normal-case text-muted-foreground/80">
            {users.length} user
          </span>
        </div>
        ...
        {!loading && users.length === 0 && (
          <EmptyState
            ...
            description={
              searching
                ? `Coba kata kunci lain selain "${q}".`
                : "Mulai berinteraksi untuk menemukan teman."
            }
          />
        )}
          ...
          {!loading &&
            users.map((u: any, i: number) => (
              ...
              >
                {!searching && i < 3 && (
                  <span
                    className={`shrink-0 size-7 rounded-full grid place-items-center text-[11px] font-black ${i === 0 ? "bg-amber-100 text-amber-700" : i === 1 ? "bg-zinc-100 text-zinc-700" : "bg-orange-100 text-orange-700"}`}
                  >
                    {i + 1}
                  </span>
                )}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[11px] text-muted-foreground flex items-center gap-1 mt-0.5">
                    <Flame className="size-3 text-orange-500" /> {u.scan_streak_current ?? 0} hari
                    streak
                  </div>
                </div>
