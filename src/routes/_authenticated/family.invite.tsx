    ...
    try {
      if (navigator.share)
        await navigator.share({
          title: "Family Plan HealthyU",
          text: "Yuk gabung family plan saya!",
          url: inviteUrl,
        });
      ...
      <main className="max-w-md mx-auto px-4 pt-4 space-y-4 animate-fade-up">
        <section className="rounded-3xl p-5 bg-gradient-to-br from-primary via-primary/85 to-accent text-primary-foreground outline-1 outline-black/5">
          ...
          <p className="text-xs opacity-80 mt-1">
            Berbagi progres, meal plan, dan dukungan dalam satu plan.
          </p>
        </section>
              ...
              <div className="space-y-3 pt-1 animate-fade-up">
                ...
                {tokenStr && (
                  <div className="flex items-center gap-2 bg-muted/40 rounded-xl px-3 py-2.5">
                    <span className="text-xs font-mono tracking-wider flex-1 truncate">
                      {tokenStr}
                    </span>
                    <button
                      onClick={() => copy(tokenStr)}
                      className="size-8 rounded-lg bg-background grid place-items-center"
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                    ...
                    className="rounded-xl bg-card outline-1 outline-black/10 py-2.5 text-xs font-semibold inline-flex items-center justify-center gap-1.5"
                  >
                    <Copy className="size-3.5" /> Salin link
                  </button>
                  <button
                    onClick={share}
                    className="rounded-xl bg-primary text-primary-foreground py-2.5 text-xs font-semibold inline-flex items-center justify-center gap-1.5"
                  >
