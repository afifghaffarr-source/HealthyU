...
const HUMAN_PROPS = /\b(?:title|placeholder|aria-label|label|alt|description)=["']([^"'<>{}\n]{3,})["']/g;
...
function walk(dir) {
  for (const entry of readdirSync(dir)) {
    ...
    else if (['.tsx', '.jsx'].includes(extname(p)) && !p.includes('.test.') && !p.includes('.spec.')) scan(p);
  }
}
...
for (const [file, items] of sorted) console.log(`  ${items.length.toString().padStart(4)}  ${file}`);
