...
| -------------- | --------------------------- | ------------------------------- |
| Event Filter   | \_\_EventFilter             | Defines the trigger (WQL query) |
| Event Consumer | \_\_EventConsumer           | Defines the action              |
| Binding        | \_\_FilterToConsumerBinding | Links filter to consumer        |
...
| -------------- | ----------------------------------------------------------------------------------------------------------------------- | --------------------------------- |
| Process Start  | SELECT \* FROM \_\_InstanceCreationEvent WITHIN 5 WHERE TargetInstance ISA 'Win32_Process'                              | Execute on specific process start |
| System Startup | SELECT \* FROM \_\_InstanceModificationEvent WITHIN 60 WHERE TargetInstance ISA 'Win32_PerfFormattedData_PerfOS_System' | Execute shortly after boot        |
| Timer-Based    | SELECT \* FROM \_\_TimerEvent WHERE TimerID='MyTimer'                                                                   | Execute at intervals              |
| User Logon     | SELECT \* FROM \_\_InstanceCreationEvent WHERE TargetInstance ISA 'Win32_LogonSession'                                  | Execute on user logon             |
